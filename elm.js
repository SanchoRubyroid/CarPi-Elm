...
Elm.CarPiBehavior = Elm.CarPiBehavior || {};
Elm.CarPiBehavior.make = function (_elm) {
   "use strict";
   _elm.CarPiBehavior = _elm.CarPiBehavior || {};
   if (_elm.CarPiBehavior.values) return _elm.CarPiBehavior.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var setReverse = F3(function (reversing,data,updatedData) {
      return reversing ? _U.eq(data.level,0) ? _U.update(updatedData,{reversedLevel: true}) : updatedData : _U.eq(updatedData.level,0) ? _U.update(updatedData,
      {reversedLevel: false}) : updatedData;
   });
   var normalizeLevel = F2(function (data,newLevel) {    return _U.update(data,{level: A3($Basics.clamp,0,100,newLevel)});});
   var increaseLevel = function (data) {    return A2(normalizeLevel,data,data.level + data.power);};
   var decreaseLevel = F2(function (data,brakeApplied) {
      var currenDataPower = brakeApplied ? data.powerMutliplier * data.power : data.power;
      return A2(normalizeLevel,data,data.level - currenDataPower);
   });
   var applyIdle = function (data) {    return _U.eq(data.level,0) ? data : A3(setReverse,false,data,A2(decreaseLevel,data,false));};
   var applyDecrease = function (data) {    return A3(setReverse,true,data,data.reversedLevel ? increaseLevel(data) : A2(decreaseLevel,data,true));};
   var applyIncrease = function (data) {    return A3(setReverse,false,data,data.reversedLevel ? A2(decreaseLevel,data,true) : increaseLevel(data));};
   var Data = F4(function (a,b,c,d) {    return {power: a,powerMutliplier: b,level: c,reversedLevel: d};});
   return _elm.CarPiBehavior.values = {_op: _op
                                      ,Data: Data
                                      ,applyIncrease: applyIncrease
                                      ,applyDecrease: applyDecrease
                                      ,applyIdle: applyIdle
...
Elm.CarPi = Elm.CarPi || {};
Elm.CarPi.make = function (_elm) {
   "use strict";
   _elm.CarPi = _elm.CarPi || {};
   if (_elm.CarPi.values) return _elm.CarPi.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   var applyBehavior = F4(function (increaseList,decreaseList,action,data) {
      ...
      function (v) {
         return _U.eq(action,v);
      },
      decreaseList) ? $CarPiBehavior.applyDecrease(data) : $CarPiBehavior.applyIdle(data);
   });
   var ReverseRight = {ctor: "ReverseRight"};
   var ReverseLeft = {ctor: "ReverseLeft"};
   var Reverse = {ctor: "Reverse"};
   var AccelerateRight = {ctor: "AccelerateRight"};
   var AccelerateLeft = {ctor: "AccelerateLeft"};
   var Accelerate = {ctor: "Accelerate"};
   ...
   var applyDirection = F2(function (action,car) {
      ...
      var updatedDirection = updatedData.reversedLevel ? Left : _U.eq(updatedData.level,0) ? Straight : Right;
   ...
   var applyTorque = F2(function (action,car) {
      ...
      return _U.update(car,{torqueLevel: updatedData.level,torqueReversed: updatedData.reversedLevel});
   });
   var update = F2(function (action,car) {    return A2(applyDirection,action,A2(applyTorque,action,car));});
   var fps = 60;
   var input = function () {
      ...
      var actions = A2($Signal.map,toAction,$Keyboard.arrows);
      var delta = $Time.fps(fps);
      return A2($Signal.sampleOn,delta,actions);
   }();
   var model = A3($Signal.foldp,update,initialCar,input);
   var externalModel = Elm.Native.Port.make(_elm).outboundSignal("externalModel",
   function (v) {
      return {torqueLevel: v.torqueLevel,torqueReversed: v.torqueReversed,directionLevel: v.directionLevel,direction: v.direction};
   },
   function () {
      ...
      var normalizedModel = function (model) {
         return {torqueLevel: model.torqueLevel
